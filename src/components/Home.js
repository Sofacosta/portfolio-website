...
const Home = () => {
  ...
  const isTestCardFlippedHandler = () => setIsTestCardFlipped(!isTestCardFlipped);
  const [isVaultCardFlipped, setIsVaultCardFlipped] = useState(false);
  const isVaultCardFlippedHandler = () => setIsVaultCardFlipped(!isVaultCardFlipped);
  const [isThreeCardFlipped, setIsThreeCardFlipped] = useState(false);
  const isThreeCardFlippedHandler = () => setIsThreeCardFlipped(!isThreeCardFlipped);
  const [isFourCardFlipped, setIsFourCardFlipped] = useState(false);
  const isFourCardFlippedHandler = () => setIsFourCardFlipped(!isFourCardFlipped);
  const [isFiveCardFlipped, setIsFiveCardFlipped] = useState(false);
  const isFiveCardFlippedHandler = () => setIsFiveCardFlipped(!isFiveCardFlipped);
